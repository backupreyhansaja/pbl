...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = $db->query("SELECT id FROM visi_misi LIMIT 1");
    $existing = $db->fetch($result);
    
    if ($existing) {
        $sql = "UPDATE visi_misi SET visi = '$visi', misi = '$misi', updated_at = CURRENT_TIMESTAMP WHERE id = {$existing['id']}";
    } else {
...
// Fetch current data
$result = $db->query("SELECT * FROM visi_misi LIMIT 1");
