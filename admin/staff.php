...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['foto']['tmp_name'], $targetPath)) {
            $foto = 'uploads/staff/' . $fileName;
        }
    }
    
    if ($id > 0) {
        $sql = "UPDATE staff SET nama = '$nama', nip = '$nip', jabatan = '$jabatan', email = '$email', phone = '$phone'";
        if ($foto) {
            $sql .= ", foto = '$foto'";
        }
        $sql .= ", updated_at = CURRENT_TIMESTAMP WHERE id = $id";
    } else {
        $sql = "INSERT INTO staff (nama, nip, jabatan, email, phone, foto) VALUES ('$nama', '$nip', '$jabatan', '$email', '$phone', '$foto')";
    }
...
// Fetch all data
$result = $db->query("SELECT * FROM staff ORDER BY id DESC");
...
if (isset($_GET['edit'])) {
    ...
    $editResult = $db->query("SELECT * FROM staff WHERE id = $editId LIMIT 1");
...
<div class="bg-white rounded-xl shadow-md p-6 mb-6">
    <h3 class="text-lg font-bold text-gray-800 mb-4">
        <?php echo $editData ? 'Edit' : 'Tambah'; ?> Data Staff
    </h3>
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <div class="flex justify-end space-x-2">
            <?php if ($editData): ?>
                <a href="staff.php" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition">
                    Batal
                </a>
            <?php endif; ?>
