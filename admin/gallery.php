...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        ...
        if ($image) {
            $sql = "INSERT INTO gallery (title, description, image, tanggal) VALUES ('$title', '$description', '$image', '$tanggal')";
        } else {
...
if (isset($_GET['edit'])) {
    ...
    $editResult = $db->query("SELECT * FROM gallery WHERE id = $editId LIMIT 1");
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <?php if ($editData && $editData['image']): ?>
            <div class="mb-4">
                ...
                <img src="../<?php echo htmlspecialchars($editData['image']); ?>" alt="Preview" class="w-48 h-48 object-cover rounded-lg">
            </div>
        <?php endif; ?>
        ...
        <?php foreach ($data as $row): ?>
            <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition">
                <div class="h-48 overflow-hidden">
                    <img src="../<?php echo htmlspecialchars($row['image']); ?>" alt="<?php echo htmlspecialchars($row['title']); ?>" class="w-full h-full object-cover">
                </div>
                <div class="p-4">
                    ...
                    <div class="flex space-x-2">
                        ...
                        <a href="?delete=<?php echo $row['id']; ?>" onclick="return confirmDelete()" class="flex-1 px-4 py-2 bg-red-500 text-white text-center rounded-lg hover:bg-red-600 transition text-sm">
