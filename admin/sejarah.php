...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = $db->query("SELECT id FROM sejarah LIMIT 1");
    $existing = $db->fetch($result);
    
    if ($existing) {
        $sql = "UPDATE sejarah SET content = '$content', updated_at = CURRENT_TIMESTAMP WHERE id = {$existing['id']}";
    } else {
        $sql = "INSERT INTO sejarah (content) VALUES ('$content')";
    }
...
// Fetch current data
$result = $db->query("SELECT * FROM sejarah LIMIT 1");
